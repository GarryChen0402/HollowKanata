...
void EnemyDashInAirState::on_enter()
{
	...
	const Character* player = CharacterManager::instance()->get_enemy();
...
void EnemyDashInAirState::on_update(float delta)
{
	...
	else if (enemy->is_on_floor())enemy->switch_state("idle");
}
...
void EnemyDashOnFloorState::on_update(float delta)
{
	...
	timer.on_update(delta);
	if (enemy->get_hp() <= 0)enemy->switch_state("dead");
	else if (!enemy->get_dashing_on_floor())enemy->switch_state("idle");
	...
	timer.set_on_timeout([&]()
		{
			...
			if (enemy->get_hp() > 5)
			{
				if (rand_num <= 25)
				{
					if (!enemy->is_on_floor())enemy->switch_state("fall");
...
void EnemyIdleState::on_enter()
{
	...
	float wait_time = 0;
	if (enemy->get_hp() > 5)wait_time = range_random(0, 2) * 0.25f;
	else wait_time = range_random(0, 1) * 0.25f;
...
void EnemyIdleState::on_update(float delta)
{
	...
	if (enemy->get_hp() <= 0)enemy->switch_state("dead");
	else if (enemy->get_velocity().y > 0)enemy->switch_state("fall");
}
...
void EnemyJumpState::on_update(float delta)
{
	...
	else if (enemy->get_velocity().y > 0)
	{
		...
		if (enemy->get_hp() > 5)
		{
			if (rand_num <= 50)enemy->switch_state("aim");
			else if (rand_num <= 80) enemy->switch_state("fall");
			else enemy->switch_state("throw_silk");
		}
		else
		{
			if (rand_num <= 50)enemy->switch_state("throw_silk");
			else if (rand_num <= 80) enemy->switch_state("fall");
			else enemy->switch_state("aim");
		}
	}
}
...
void EnemyRunState::on_update(float delta)
{
	...
	else if (abs(pos_enemy.x - pos_player.x) < MIN_DIS)
	{
		...
		if (enemy->get_hp() > 5)
		{
			if (rand_num <= 75)enemy->switch_state("squat");
			else enemy->switch_state("throw_silk");
		}
		else
		{
			if (rand_num <= 75)enemy->switch_state("throw_silk");
...
EnemyThrowSwordState::EnemyThrowSwordState()
{
	...
	timer_throw.set_on_timeout([&]()
		{
			std::cout << "Before throw sword" << std::endl;
			Enemy* enemy = (Enemy*)CharacterManager::instance()->get_enemy();
			enemy->throw_sword();
	...
	timer_switch.set_on_timeout([&]()
		{
			...
			if (enemy->get_hp() > 5)
			{
				if (rand_num <= 50)enemy->switch_state("squat");
				else if (rand_num <= 80)enemy->switch_state("jump");
				else enemy->switch_state("idle");
			}
			else
			{
				if (rand_num <= 50)enemy->switch_state("jump");
				else if (rand_num <= 80)enemy->switch_state("throw_silk");
				else enemy->switch_state("idle");
			}
...
void EnemyThrowSwordState::on_update(float delta)
{
	...
	std::cout << "throw sword on update start" << "\t";
	Enemy* enemy = (Enemy*)CharacterManager::instance()->get_enemy();
	...
	if (enemy->get_hp() <= 0)enemy->switch_state("dead");
	std::cout << "throw sword on update end" << "\n";
}
