...
{
	if (!manager) manager = new ResourcesManager();
	return manager;
}
...
void ResourcesManager::load()
{
	for (const auto& info : image_info_list)
	{
		...
		if (!check_image_valid(image))throw info.path;

		image_pool[info.id] = image;
		//std::cout << info.id << " " << image->getwidth() << " " << image->getheight() << std::endl;
	}
	...
	for (const auto& info : atlas_info_list)
	{
		...
		//std::cout << info.id << " " << atlas->get_size() << std::endl;
	}
