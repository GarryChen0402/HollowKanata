...
Character::Character()
{
	...
	timer_invulnerable_blink.set_wait_time(0.0075f);
...
void Character::on_render()
{
	...
	if (is_facing_left) {
		std::cout << "left render start" << std::endl;
		current_animation->left.on_render();
		std::cout << "left render complete" << std::endl;
	}
	else {
		std::cout << "right render start" << std::endl;
		//current_animation->right.
		current_animation->right.on_render();
		std::cout << "right render complete" << std::endl;
	}
...
void Character::set_animation(const std::string& id)
{
	...
	std::cout << id << " " << current_animation->left.get_frame_list_size()<<" \t";
	std::cout << id << " " << current_animation->right.get_frame_list_size() << std::endl;;
