...
Enemy::Enemy()
{
	...
	animation_squat_left.set_loop(true);
	...
	animation_squat_right.set_loop(true);
	...
	Animation& animation_dash_in_air_left_vfx = animation_dash_in_air_vfx.left;
	animation_dash_in_air_left_vfx.set_interval(0.1f);
	animation_dash_in_air_left_vfx.set_loop(false);
	animation_dash_in_air_left_vfx.set_anchor_mode(Animation::AnchorMode::Centered);
	animation_dash_in_air_left_vfx.add_frame(ResourcesManager::instance()->find_atlas("enemy_vfx_dash_in_air_left"));

	Animation& animation_dash_in_air_right_vfx = animation_dash_in_air_vfx.right;
	animation_dash_in_air_right_vfx.set_interval(0.1f);
	animation_dash_in_air_right_vfx.set_loop(false);
	animation_dash_in_air_right_vfx.set_anchor_mode(Animation::AnchorMode::Centered);
	animation_dash_in_air_right_vfx.add_frame(ResourcesManager::instance()->find_atlas("enemy_vfx_dash_in_air_right"));


	Animation& animation_dash_on_floor_left_vfx = animation_dash_on_floor_vfx.left;
	animation_dash_on_floor_left_vfx.set_interval(0.1f);
	animation_dash_on_floor_left_vfx.set_loop(false);
	animation_dash_on_floor_left_vfx.set_anchor_mode(Animation::AnchorMode::BottomCentered);
	animation_dash_on_floor_left_vfx.add_frame(ResourcesManager::instance()->find_atlas("enemy_vfx_dash_on_floor_left"));

	Animation& animation_dash_on_floor_right_vfx = animation_dash_on_floor_vfx.right;
	animation_dash_on_floor_right_vfx.set_interval(0.1f);
	animation_dash_on_floor_right_vfx.set_loop(false);
	animation_dash_on_floor_right_vfx.set_anchor_mode(Animation::AnchorMode::BottomCentered);
	animation_dash_on_floor_right_vfx.add_frame(ResourcesManager::instance()->find_atlas("enemy_vfx_dash_on_floor_right"));
...
{
	if (velocity.x >= 0.0001f)is_facing_left = (velocity.x < 0);
...
void Enemy::throw_sword()
{
	...
	std::cout << "Throw a new sword, the existed sword count is : " << sword_list.size()<<std::endl;
	sword_list.push_back(new_sword);
}

void Enemy::on_dash()
{
	if (is_dashing_in_air)current_dash_animation = velocity.x < 0 ? &animation_dash_in_air_vfx.left : &animation_dash_in_air_vfx.right;
	else current_dash_animation = velocity.x < 0 ? &animation_dash_on_floor_vfx.left : &animation_dash_on_floor_vfx.right;
