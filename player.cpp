...
Player::Player()
{
	...
	animation_slash_down.set_all_data(
		0.07f,
		false,
		Animation::AnchorMode::Centered,
		ResourcesManager::instance()->find_image("player_vfx_attack_down"), 5
	);

	animation_slash_left.set_all_data(
		0.07f,
		false,
		Animation::AnchorMode::Centered,
		ResourcesManager::instance()->find_image("player_vfx_attack_left"), 5
	);

	animation_slash_right.set_all_data(
		0.07f,
		false,
		Animation::AnchorMode::Centered,
		ResourcesManager::instance()->find_image("player_vfx_attack_right"), 5
	);

	animation_jump_vfx.set_all_data(
		0.05f,
		false,
		Animation::AnchorMode::BottomCentered,
		ResourcesManager::instance()->find_image("player_vfx_jump"), 5,
		[&]() {
			is_jump_vfx_visible = false;
		}
	);

	animation_land_vfx.set_all_data(
		0.1f,
		false,
		Animation::AnchorMode::BottomCentered,
		ResourcesManager::instance()->find_image("player_vfx_land"), 2,
		[&]() {
...
{
	if (hp > 0 && !is_rolling)velocity.x = get_move_axis() * SPEED_RUN;
...
{
	if (is_jump_vfx_visible)animation_jump_vfx.on_render();

	if (is_land_vfx_visible)animation_land_vfx.on_render();

	Character::on_render();

	if (is_attacking) current_slash_animation->on_render();
}
...
void Player::update_attack_dir(int x, int y)
{
	...
	if (angle >= -PI / 2 && angle < PI / 4)attack_dir = AttackDir::Right;
	else if (angle >= PI / 4 && angle < 3 * PI / 4)attack_dir = AttackDir::Down;
	else if (abs(angle) >= 3 * PI / 4 && abs(angle) <= PI)attack_dir = AttackDir::Left;
