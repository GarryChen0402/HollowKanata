...
{
	if (stage == Stage::Idle)timer_idle.on_update(delta);
	if (stage == Stage::Aim)timer_aim.on_update(delta);
	...
	case Barb::Stage::Dash:
		...
		if (current_position.y >= CharacterManager::instance()->get_player()->get_floor_y())on_break();
		if (current_position.y <= 0)is_valid = false;
		break;
	}
